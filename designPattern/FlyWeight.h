...
#include<string>
	...
	{
	private:
		std::map<std::string, Flyweight*>flyweights;//模块名列表
	public:
		 ...
		 */
		Flyweight* GetFlyWeight(std::string key)//工厂返回享元对象，而这些对象的属性都存放在一个该工厂的模块名列表中
		{
		...
		std::map<std::string, Flyweight*>::size_type countN()//返回当前有多少个可供共享的模块
		{

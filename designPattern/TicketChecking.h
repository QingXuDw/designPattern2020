...
public:
	 ...
	 */
	void Check() {
		string itemID;										//使用此系统的工作人员所属的项目编号
		string itemName[] = {"冰封鬼窟", "登山", "动物园"};	//使用此系统的工作人员所属的项目名称
		string ticketID;									//所检查的套票编号
		string d;			//用于吞没多余输入内容的字符串
		...
		while (1) {
			...
			if (itemID.length() == 1 && itemID >= "1" && itemID <= "3") {		//若输入的项目编号合法则欢迎并引导工作人员至下一步
				...
				cout << "  欢迎你, " << itemName[id] << " 的工作人员！" << endl;
				break;				//项目编号获取成功，跳出循环
			}
			else 
				cout << "  项目编号错误，请重新输入: ";		//若输入的项目编号不合法则继续循环
		}

		while (1) {
			cout << "  请输入游客套票的四位编码(以空格分隔)(以quit退出): ";
			cin >> ticketID;
	...
	void Content_Cave(string type, int status) {
		cout << "  此位游客购买了" << type << ", ";		//向工作人员输出当前游客套票中与自己所属项目的内容
		switch (status) {
		case 0:
			cout << "但未选择本项目。" << endl;
			break;
		case 1:
			cout << "并选择了本项目， 您可以引导此位游客开始冰封鬼窟项目的游玩！" << endl;
			break;
		default:
			cout << "但编码错误，请重新输入！" << endl;
		}
	}
	...
	void Content_Mountain(string type, int status) {
		...
		case 0:
			...
			break;
		case 1:
			cout << "并选择了本项目, 同时选择了初级套餐， 您可以引导此位游客开始体验登山！" << endl;
			break;
		case 2:
			cout << "并选择了本项目, 同时选择了高级套餐， 您可以引导此位游客开始体验登山！" << endl;
			break;
		case 3:
			cout << "并选择了本项目, 同时选择了专业套餐， 您可以引导此位游客开始进行登山！" << endl;
			break;
		default:
			cout << "但编码错误，请重新输入！" << endl;
		}
	...
	void Content_Zoo(string type, int status) {
		...
		case 0:
			...
			break;
		case 1:
			cout << "并选择了本项目, 同时选择了普通参观， 您可以引导此位游客进入动物园！" << endl;
			break;
		case 2:
			cout << "并选择了本项目, 同时选择了基础参观+纪念品赠送， 您可以引导此位游客进入动物园！" << endl;
			break;
		case 3:
			cout << "并选择了本项目, 同时选择了基础参观+主题餐厅就餐， 您可以引导此位游客进入动物园！" << endl;
			break;
		default:
			cout << "但编码错误，请重新输入！" << endl;
		}
